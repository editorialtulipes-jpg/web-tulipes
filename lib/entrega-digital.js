...
async function enviarLibrosDigitales({ email, items, libros, projectRoot }) {
  ...
  const encontrados = items
    ...
    .filter((libro) => libro && libro.archivo_digital);

  if (encontrados.length === 0) return;

  const attachments = encontrados.map((libro) => ({
    filename: path.basename(libro.archivo_digital),
    content: fs.readFileSync(path.join(projectRoot, libro.archivo_digital)).toString("base64"),
  }));

  const titulos = encontrados.map((libro) => libro.titulo).join(", ");

  const { error } = await resend.emails.send({
    from: "Editorial Tulipes <onboarding@resend.dev>",
    to: email,
    subject: "Tu libro digital — Editorial Tulipes",
    html: `<p>¡Gracias por tu compra! Adjunto encontrarás: <strong>${titulos}</strong>.</p>`,
    attachments,
  });
